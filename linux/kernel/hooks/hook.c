...
#ifdef CONFIG_X86_64
	if (!hook_insn_rip_relative(src_insn))
		return;
...
static inline pte_t *hook_lookup_address(unsigned long address, unsigned int *level)
{
...
#else
#define hook_lookup_address(address, level) lookup_address(address, level);
#endif
...
static inline void set_addr_rw(unsigned long addr, bool *flag) {
	...
	*flag = true;
	...
	pte = hook_lookup_address(addr, &level);
...
int symbol_hijack(struct kernsym *sym, const char *symbol_name, unsigned long *code) {
	...
	while (orig_addr < end_addr) {
		hook_insn_init(&insn, (void *)orig_addr);
		hook_insn_get_length(&insn);
		if (insn.length == 0) {
			printk(PKPRE
